...
export class BoardService
{
  ...
  async create(workSpaceId: number, userId: number, title: string, image_path: string)
  {
    ...
    const workspace = await this.workspaceService.findOneByWorkspaceIdAndUserId(workSpaceId, userId);
  ...
  async AddMember(boardId: number, email: string, role: BoardMemberRole)
  {
    ...
    // userId 값만
    const w_Members = getWorkSpaceMember.map(member => member.user).map(member => member.userId)
